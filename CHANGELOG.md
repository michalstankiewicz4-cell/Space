...
### Fixed
- Things visibly "popping in" after opening/refreshing the page: the
  start screen first showed fallback fonts, English text (for Polish
  players) and flat buttons, fixing themselves ~1.5s later. The cause was
  the 3D scene setup blocking the browser. Now:
  - the start screen is set up and painted *before* the 3D scene is built;
  - fonts are self-hosted (`fonts/`, `css/fonts.css`) instead of loaded
    from Google Fonts, and the start-screen ones are preloaded;
  - with a saved non-English language, the start panel stays hidden until
    it's translated instead of flashing English first;
  - the button grain is a static image (`css/ui/grain.png`) instead of
    being generated by JS.
